...
export default function App() {
  ...
  //   const storedValue = JSON.parse(localStorage.getItem("movie"));
  //   return storedValue;
  // });
  ...
  function handleAddWatched(movie) {
    ...
    // localStorage.setItem("movie", JSON.stringify([...watched, movie]));
  }
...
function WatchedMovieLists({ watched, onDelete }) {
  return (
    <ul className="list">
      {watched.map((movie) => (
...
function MovieDetails({ selectedId, onClose, onAddWatched, watched }) {
  ...
  // if (imdbRating > 8) return <p>leggo</p>;
    ...
    function () {
      async function getMovieDetails() {
        try {
          ...
          if (data.Response === "False") throw new Error("Movie not loaded");
          // COming back to check if this actually worked!
          setMovie(data);
        } catch (err) {
          setError(err);
        } finally {
  ...
  useEffect(
    function () {
      if (!title) return;
      document.title = loading ? "Loading..." : `MOVIE | ${title}`;
      ...
      {error ? (
        ...
        <>
          <header>
            ...
            <img src={poster} alt={`Poster =f ${movie}`} />
          ...
          <section>
            {isWatched ? (
              <p>
                You have rated this movie <span>{watchedUserRating}✨</span>
              </p>
            ) : (
              <>
